...
def insert_bact_kernel(mask_volume,
    ...
    plane_size = int(x_plane_size) * int(y_plane_size)
    total = plane_size * int(z_plane_size)
    ...
    if tid < total:
        ...
        z = tid // int(plane_size)
        y = (tid % int(plane_size)) // int(x_plane_size)
        x = tid % int(x_plane_size)
...
def GPU_insert_bact_in_mask_volume(mask_volume, bact, vox_size_xy, vox_size_z):
    ...
    total_voxels = int(x_plane_size) * int(y_plane_size) * int(z_plane_size)
    nthread = 1024
    ...
    insert_bact_kernel[nBlock, nthread](
        ...
        m1_x, m1_y, m1_z,
        m2m1_x, m2m1_y, m2m1_z,
        int(i_x_min), int(i_y_min), int(i_z_min),
        vox_size_xy, vox_size_z,
        bact.thickness * 0.5,
        int(x_plane_size), int(y_plane_size), int(z_plane_size)
    )
